...
@else
        ...
        <form wire:submit.prevent="saveGroup" autocomplete="off">
            ...
            <div class="form-group">
                <label>{{trans('create-group-service.name_group')}}</label>
                <input wire:model="name_group" type="text" name="name_group" class="form-control" required>
            </div>

            <div class="form-group">
                <label>{{trans('create-group-service.notes')}}
                    </label>
                ...
                <div class="card-header">
                    <div class="col-md-12">
                        <button class="btn btn-outline-primary"
                                wire:click.prevent="addService">{{trans('create-group-service.addService')}}
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr class="table-primary">
                                <th> {{trans('create-group-service.nameService')}}</th>
                                <th width="200">

                                {{trans('create-group-service.number')}}
                                
                                </th>
                                <th width="200">{{trans('create-group-service.operation')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <td>
                                        @if($groupItem['is_saved'])
                                            <button class="btn btn-sm btn-primary"
                                                    ...
                                                    {{trans('create-group-service.edit')}}
                                            </button>
                                        @elseif($groupItem['service_id'])
                                            <button class="btn btn-sm btn-success mr-1"
                                                    wire:click.prevent="saveService({{$index}})">
                                                    {{trans('create-group-service.submit')}}
                                                    
                                            </button>
                                        @endif
                                        <button class="btn btn-sm btn-danger"
                                                wire:click.prevent="removeService({{$index}})">{{trans('create-group-service.delete')}}
                                        </button>
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <div class="col-lg-4 ml-auto text-right">
                        <table class="table pull-right">
                            <tr>
                                <td style="color: red">    {{trans('create-group-service.subtotal')}}
                                                    </td>
                                <td>{{ number_format($subtotal, 2) }}</td>
                            </tr>

                            <tr>
                                <td style="color: red">  {{trans('create-group-service.discount_value')}} </td>
                                <td width="125">
                            ...
                            <tr>
                                <td style="color: red">{{trans('create-group-service.taxes')}} </td>
                                <td>
                            ...
                            <tr>
                                <td style="color: red">{{trans('create-group-service.total')}} 
                             </td>
                    ...
                    <br/>
                    <div>
                        <input class="btn btn-outline-success" type="submit" value="{{trans('create-group-service.submit')}} ">
                    </div>
                </div>
            </div>
